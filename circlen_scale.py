...
from .mode_enums import CNScaleMode, CommandMode
...
class Pref:
    circle_back_color = (0.2,0.5,0.8,0.3)
    sphere_line_color = (.6,.5,.5,1)
    n_line_color = (.3,.3,.3,1)
    mos_point_color = (0.1,0.6,1,1)

class DrawRequests_Scale:
    def __init__(self):
...
class CircleNScalePanel(bpy.types.Panel):
    bl_context = ""
    bl_region_type = "TOOLS"; bl_space_type = "VIEW_3D"; bl_category = "Tools"; bl_label = "Circle N Scale"
    def draw(self, context): layout = self.layout; layout.operator(CircleNScaleOperator.bl_idname, text=CircleNScaleOperator.bl_label)

class CircleNScaleOperator(bpy.types.Operator, DynamicMemberSetHelperMixin):
    bl_idname = "view3d.circle_n_scale"
    bl_label = "Circle N Scale"
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            elif isinstance(mode_next, CNScaleMode):
...
def pre_begin_of_modal(self, context, event, subject):
    ...
    self.draw_requests3d = DrawRequests_Scale()

    CircleNScaleOperator.bgl_draw_handler = bpy.types.SpaceView3D.draw_handler_add(bgl_draw_callback, (self.draw_requests3d,), 'WINDOW', 'POST_VIEW')
    self.initv('check__bgl_draw_handler', True)

    modalvar.initv('prev_orientation', context.area.spaces[0].transform_orientation)
...
def pre_end_of_modal(self, context, event):
    ...
    context.area.spaces[0].transform_orientation = self.modalvar.prev_orientation

    if self.existv('check__bgl_draw_handler'):
        bpy.types.SpaceView3D.draw_handler_remove(CircleNScaleOperator.bgl_draw_handler, 'WINDOW')
...
class CommandParser:
    ...
    def get_mode(self, event):
        mode = CommandMode.PASS

        if self.force_next is not None:
            mode = self.force_next
            self.force_next = None
            return mode
        ...
        if ctrl_down and event.type == 'Z':
            return CommandMode.PREVENT_DEFAULT
        if event.type == 'C':
            self.force_next = CNScaleMode.DIR
            return CNScaleMode.CAPTURE_N

        elif left_press:
            return CommandMode.FINISH

        elif right_press and shift_down:
            return CNScaleMode.UV_SCALE
        elif right_press:
            return CNScaleMode.N_SCALE

        elif right_release and shift_down:
            return CNScaleMode.DIR
        elif right_release:
            return CNScaleMode.DIR
...
def modal_redraw_main(context, event, draw_requests3d, modalvar):
    ...
    if in_transform.mode_prev != modalvar.sphere_mode:
        ...
        if in_transform.mode_prev == CNScaleMode.DIR:
        ...
        if modalvar.sphere_mode == CNScaleMode.N_SCALE:
            set = Object()
            p = subject.get_pivot()
            set.begin = p.copy()
            set.end = p + in_transform.n*10*radius
            draw_requests3d.n_set = set
        elif modalvar.sphere_mode == CNScaleMode.UV_SCALE:
            p = subject.get_pivot()
    ...
    if modalvar.sphere_mode == CNScaleMode.DIR:
    ...
    elif modalvar.sphere_mode == CNScaleMode.CAPTURE_N:
    ...
    elif modalvar.sphere_mode == CNScaleMode.N_SCALE:
        base_pivot = subject.get_pivot()

        n = in_transform.n
        u,v = calc_circle_directions_major_minor(context, base_pivot, n)
        reset_orientation(context, u, v, n)

        p2d = view_3d_to_region_2d(context, base_pivot)
        val =(mouse_r2d - p2d).length / (in_transform.prev_mouse_r2d - p2d).length
        pe_sets = get_proportional_edit_settings()
        bpy.ops.transform.resize(value=(1,1,val), constraint_orientation=SCALE_ORIENTATION_NAME, constraint_axis=(False, False, True)
        ...
        val =(mouse_r2d - p2d).length / (in_transform.prev_mouse_r2d - p2d).length
        pe_sets = get_proportional_edit_settings()
        bpy.ops.transform.resize(value=(val,val,1), constraint_orientation=SCALE_ORIENTATION_NAME, constraint_axis=(True, True, False)
                                , proportional=pe_sets.proportional
                                , proportional_edit_falloff=pe_sets.proportional_edit_falloff
                                , proportional_size=pe_sets.proportional_size
                                )
...
SCALE_ORIENTATION_NAME = 'Circle N Scale'
def reset_orientation(context, n_x, n_y, n_z):
    bpy.ops.transform.create_orientation(name = SCALE_ORIENTATION_NAME, overwrite = True)
    if context.mode in ['EDIT_CURVE', 'EDIT_SURFACE']:
        bpy.ops.transform.create_orientation(name = SCALE_ORIENTATION_NAME, overwrite = True, use_view=True)
    context.scene.orientations[SCALE_ORIENTATION_NAME].matrix = (n_x, n_y, n_z)
    context.area.spaces[0].transform_orientation = SCALE_ORIENTATION_NAME
...
def get_subject(context):
    ...
    if context.mode == 'EDIT_MESH':
        ...
        subject = ScaleSubjectMesh(context.edit_object)
        return subject
        ...
        subject = ScaleSubjectObject(context.selected_objects)
        return subject

    elif context.mode == 'EDIT_ARMATURE':
        if len( [ eb for eb in context.edit_object.data.edit_bones if eb.select_head or eb.select_tail ] ) == 0:
            return None
        subject = ScaleSubjectArmature(context.edit_object)
        return subject
        ...
        subject = ScaleSubjectPoseBone(context.active_object, sel_pose_bones)
        return subject
        pass
    elif context.mode in ['EDIT_CURVE', 'EDIT_SURFACE']:
        subject = ScaleSubjectCurve(context.active_object)
        return subject

    return None

class ScaleSubjectMesh(SubjectStateMesh):
    pass

class ScaleSubjectObject(SubjectStateObject):
    pass

class ScaleSubjectArmature(SubjectStateArmature):
    pass

class ScaleSubjectPoseBone(SubjectStatePoseBone):
    pass

class ScaleSubjectCurve(SubjectStateCurve):
    pass

def register():
    bpy.utils.register_class(CircleNScaleOperator)

def unregister():
    bpy.utils.unregister_class(CircleNScaleOperator)
